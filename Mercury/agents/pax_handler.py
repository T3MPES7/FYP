...
class PaxHandler(Agent):
	...
	def register_pax_group_flight2train(self, pax):
		...
		# Log the registration event for flight-to-train connection
		# used for data mining purposes
		logger.info(
			"Registered passenger for flight-to-train connection. Passenger ID: %s",
			pax.id,
			extra={
				"env_time": self.env.now,
				"event_type": "PaxRegistration_Flight2Train",
				"details": {
					"passenger_id": pax.id,
					"itinerary": pax.itinerary,
					"status": pax.status,
					"pax_type": pax.pax_type,
					"action": "Register Pax Group Flight2Train"
				}
			}
		)
	...
	def register_pax_group_train2flight(self, pax):
		...
		# Log the registration event for train-to-flight connection
		# used for data mining purposes
		logger.info(
			"Registered passenger for train-to-flight connection. Passenger ID: %s",
			pax.id,
			extra={
				"env_time": self.env.now,
				"event_type": "PaxRegistration_Train2Flight",
				"details": {
					"passenger_id": pax.id,
					"itinerary": pax.itinerary,
					"status": pax.status,
					"pax_type": pax.pax_type,
					"action": "Register Pax Group Train2Flight"
				}
			}
		)
...
class Air2RailHandler(Role):
	...
	def check_flight2rail_connection_request(self, pax_id, flight2rail_connection_request, received_gate2kerb_time_estimation_event, received_ground_mobility_estimation_event, received_estimated_train_departure_event):
		...
		# logging for data mining purposes
		#capturing final status for passangers
		logger.info(
			"Final status for passenger: %s",
			pax_id,
			extra={
				"env_time": self.agent.env.now,
				"event_type": "Flight2Rail_Final",
				"details": {
					"passenger_id": pax_id,
					"itinerary": self.agent.pax_info_post[pax_id]['pax'].itinerary,
					"status": self.agent.pax_info_post[pax_id]['pax'].status,
					"pax_type": self.agent.pax_info_post[pax_id]['pax'].pax_type,
					"delay": delay,
				}
			}
		)
	...
	def request_estimated_train_departure(self, train_uid, stop_id, pax_id, train_operator_uid):
		...
		#integrating logging capabilities for data mining purposes
		#capturing requested estimated train dparture
		logger.info(
			"Sent estimated train departure times request for passenger: %s",
			pax_id,
			extra={
				"env_time": self.agent.env.now,
				"event_type": "TrainDeparture_Request",
				"details": {
					"passenger_id": pax_id,
					"train_uid": train_uid,
					"stop_id": stop_id,
					"train_operator_uid": train_operator_uid,
					"action": "Request Estimated Train Departure"
				}
			}
		)
...
class Rail2AirHandler(Role):
	...
	def check_rail2flight_connection_request(self, pax_id, rail2flight_connection_request):
		...
		#logging call that captures the final state.
		#used for data mining purposes
		logger.info(
			"Final status for passenger: %s, Status: %s",
			pax_id,
			pax.status,
			extra={
				"env_time": self.agent.env.now,
				"event_type": "Rail2Air_Final",
				"details": {
					"passenger_id": pax_id,
					"itinerary": pax.itinerary,
					"status": pax.status,
					"time_at_gate": pax.time_at_gate,
					"in_transit_to": pax.in_transit_to,
					"action": "Completed Rail2Air Handling"
				}
			}
		)
...
class PassengerReallocation(Role):
	...
	def do_reallocation(self, pax_id, origin, destination, from_time, train_operator_uid, gtfs_name):
		...
		# logging call summarizing the final reallocation outcome.
		#used for data mining purposes
		logger.info(
			"Do reallocation complete for passenger: %s",
			pax_id,
			extra={
				"env_time": self.agent.env.now,
				"event_type": "PaxReallocation",
				"details": {
					"passenger_id": pax_id,
					"itinerary": pax.itinerary,
					"status": pax.status,
