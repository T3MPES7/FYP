...
class ProcessTurnaround(Role):
	...
	def do_turnaround(self, aircraft, tt):
		...
		# logging.info(
        #     f"[{self.agent.icao}] {aircraft} resource released at t={self.agent.env.now}."
        # )
		##
	...
	def wait_for_turnaround_request(self, msg):
		...
		##logging capabilities
		# logging.info(
        #     f"[{self.agent.icao}] Turnaround request from={msg['from']} "
        #     f"flight={msg['body']['flight_uid']}, AC={msg['body']['aircraft']}."
        # )
		##
...
class TaxiOutEstimator(Role):
	...
	def return_taxi_out_estimation(self, flight_uid, toe):
		##logging capablities
		# logging.info(
        #     f"[{self.agent.icao}] Sending taxi-out estimation={toe:.2f} to flight={flight_uid}."
        # )
		##
		msg_back = Letter()
...
class TaxiOutProvider(Role):
	...
	def return_taxi_out_time(self, flight_uid, to):
		##logging capablities
		# logging.info(
        #     f"[{self.agent.icao}] Sending final taxi-out={to:.2f} to flight={flight_uid}."
        # )
		##
		msg_back = Letter()
		msg_back['to'] = flight_uid
...
class TaxiInProvider(Role):
	...
	def wait_for_taxi_in_request(self, msg):
		...
		##logging capablities
		# logging.info(
        #     f"[{self.agent.icao}] Computed actual taxi-in={ti:.2f} for flight={msg['from']}."
        # )
		##
	...
	def return_taxi_in_time(self, flight_uid, ti):
		##logging capablities
		# logging.info(
        #     f"[{self.agent.icao}] Sending final taxi-in={ti:.2f} to flight={flight_uid}."
        # )
		##
		msg_back = Letter()
		msg_back['to'] = flight_uid
