...
#Integrating logging capabilities to the agent
import logging
	...
	def consider_waiting_pax(self, flight_uid, missing_pax):
		...
		if num_missing_pax_groups > 0:
			...
			if wait_time > 0:
				...
				#integrating logging capablities for the agent
				#capturing Flight re-time departure for missing pax
				new_eobt = current_eobt + wait_time
				logger.info(
					"Flight re-times departure to wait for missing pax",
					extra={
						"env_time": self.agent.env.now,
						"flight_uid": flight_uid,
						"wait_time": wait_time,
						"old_EOBT": current_eobt,
						"new_EOBT": new_eobt,
						"event_type": "PassengerImpact",
						"action": "Flight re-time departure for missing pax"
					}
				)
...
class DynamicCostIndexComputer(Role):
	...
	def request_potential_delay_recovery_info(self, flight_uid, use_dci_landing=True):
		...
		#integrating logging capablities for the agent
		#capturing Requested Potential Delay Recovery Info event
		logger.info(
			"Requested potential delay recovery info for flight %s with use_dci_landing=%s.",
			flight_uid,
			use_dci_landing,
			extra={
				"env_time": self.agent.env.now,
				'event_type': 'DelayRecovery',
				'details': {
					'flight_number': flight_uid,
					'use_dci_landing': use_dci_landing,
					'action': 'Requested Potential Delay Recovery Info'
				}
			}
		)
		##
	...
	def wait_for_potential_delay_recovery_info(self, msg):
		...
		#Integrating logging capabilities for the agent
		#capturing Received Potential Delay Recovery Info event
		logger.info(
            "Received potential delay recovery info for flight %s.",
            self.agent.uid,
            extra={
				"env_time": self.agent.env.now,
                'event_type': 'DelayRecovery',
                'details': {
                    'flight_number': self.agent.uid,
                    'action': 'Received Potential Delay Recovery Info'
                }
            }
        )
        ##
	...
	def cost_index_assessment(self, flight_uid, push_back_ready_event):
		...
		#Integrating logging capabilities for the agent
		#capturing Received TOC Reached Message event
		logger.info(
            "Received TOC reached message for flight %s.",
            self.agent.uid,
            extra={
				"env_time": self.agent.env.now,
                'event_type': 'DelayRecovery',
                'details': {
                    'flight_number': self.agent.uid,
                    'action': 'Received TOC Reached Message'
                }
            }
        )
	...
	def decide_if_delay_recovery_performed(self, delay):
		...
		#Integrating logging capabilities for the agent
		#capturing Decided on Delay Recovery event
		logger.info(
			"Decision on delay recovery for flight %s with delay %s minutes: %s.",
			self.agent.uid,
			delay,
			'Performed' if self.delay_recovery else 'Not Performed',
			extra={
				"env_time": self.agent.env.now,
				'event_type': 'DelayRecovery',
				'details': {
					'flight_number': self.agent.uid,
					'delay': delay,
					'delay_recovery_performed': self.delay_recovery,
					'action': 'Decided on Delay Recovery'
				}
			}
		)
		##
	...
	def build_delay_cost_functions_dci_l1_old(self, flight_uid, diff=True, up_to_date_baseline=False):
		...
		#Integrating logging capabilities for the agent
		#capturing Built old delay cost functions for flight
		logger.info(
			"Built old delay cost functions for flight %s with diff=%s and up_to_date_baseline=%s.",
			flight_uid,
			diff,
			up_to_date_baseline,
			extra={
				...
				'details': {
					'flight_number': flight_uid,
					'diff': diff,
					'up_to_date_baseline': up_to_date_baseline,
					'action': 'Built Old Delay Cost Functions'
				}
			}
		)
		##
	...
	def build_delay_cost_functions_dci_l1(self, flight_uid, diff=True, up_to_date_baseline=False):
		...
		#Integrating logging capabilities for the agent
		#capturing Built delay cost functions for flight
		logger.info(
			"Built delay cost functions for flight %s with diff=%s and up_to_date_baseline=%s.",
			flight_uid,
			diff,
			up_to_date_baseline,
			extra={
				...
				'details': {
					...
					'diff': diff,
					'up_to_date_baseline': up_to_date_baseline,
					'action': 'Built Delay Cost Functions'
				}
			}
		)
        ##
	...
	def reassess_cost_index_TA0(self, flight_uid):
		...
		##logging capabilities
			logging.info(
				"Flight %s top-of-climb speed-up: recovering=%s min, speed_factor=%.2f, extra_fuel=%.1f",
				flight_uid, delta_t, perc_selected, dfuel
			)
		##
		...
		#Integrating logging capabilities for the agent
		#capturing Reassessed cost index TA0 for flight
		logger.info(
            "Reassessed cost index TA0 for flight %s.",
            flight_uid,
            extra={
				"env_time": self.agent.env.now,
                'event_type': 'Cost',
                'details': {
                    'flight_number': flight_uid,
                    'action': 'Reassessed Cost Index TA0'
                }
            }
        )
        ##
	...
	def reassess_cost_index_TA1(self, flight_uid):
		...
		#Integrating logging capabilities for the agent
		#capturing Reassessed cost index TA1 for flight
		logger.info(
			"Reassessed cost index TA1 with new parameters.",
			flight_uid, delta_t, perc_selected, dfuel,
			extra={
				"env_time": self.agent.env.now,
				'event_type': 'CostAdjustment',
				'details': {
					'flight_number': flight_uid,
					'recovering_minutes': delta_t,
					'speed_factor': perc_selected,
					'extra_fuel_kg': dfuel,
					'action': 'Reassessed Cost Index TA1',
					'new_parameters': {
						'estimated_delay': estimated_delay,
						'delta_time': -delta_t,
						'percentage_selected': perc_selected,
						'fuel_delta': dfuel,
						'extra_fuel_available': extra_fuel_available,
	...
	def reassess_cost_index_TA2(self, flight_uid):
		...
		##Pluslogging 
		action_taken = 'NoneNeeded'
		speed_adjustment_details = {}
		##
		...
		if exp_arr_delay > 0:
			...
			if (tfsc['time_fuel_func'] is not None) and (tfsc['time_zero_fuel'] is not None) and (tfsc['max_time_w_fuel'] > 0):
				...
				##PlusLogging
				speed_adjustment_details = {
					'recovery_minutes': delta_t,
					'speed_adjustment_percent': perc_selected,
					'fuel_saved_kg': dfuel,
					'extra_fuel_available_kg': extra_fuel_available
            	}
		...
		#Integrating logging capabilities for the agent
		#capturing Reassessed cost index TA2 for flight
		logger.info(
			"Reassessed cost index TA2 for flight %s: %s",
			flight_uid,
			action_taken,
			extra={
				"env_time": self.agent.env.now,
				'event_type': 'CostAdjustment',
				'flight_uid': flight_uid,
				'expected_arrival_delay_minutes': exp_arr_delay,
				'action_taken': action_taken,
				'details': speed_adjustment_details
			}
		)
        ##
	...
	def send_speed_up_msg_to_fpu(self, flight_uid, perc_selected, change_tod_dci=True):
		...
		#Integrating logging capabilities for the agent
		#capturing Sent speed-up message to FPU for flight
		logger.info(
            "Sent speed-up message to FPU for flight %s with percentage %s%% and change_tod_dci=%s.",
            flight_uid,
            perc_selected,
            change_tod_dci,
            extra={
				"env_time": self.agent.env.now,
                'event_type': 'FlightPlan',
                'details': {
                    'flight_number': flight_uid,
                    'speed_percentage': perc_selected,
                    'change_tod_dci': change_tod_dci,
                    'action': 'Sent Speed-Up Message to FPU'
                }
            }
        )
        ##
	...
	def save_dci_decision_info(self, flight_uid, params):
		...
		#Integrating logging capabilities for the agent
		#capturing Saved DCI decision info for flight
		logger.info(
            "Saved DCI decision info for flight %s with parameters: %s.",
            flight_uid,
            params,
            extra={
				"env_time": self.agent.env.now,
                ...
                'details': {
                    'flight_number': flight_uid,
                    'dci_parameters': params,
                    'action': 'Saved DCI Decision Info'
                }
            }
        )
        ##
...
class PassengerReallocation(Role):
	...
	def compute_missed_connecting_pax(self, flight_uid):
		...
		#Integrating logging capabilities for the agent
		#capturing Computed missed connecting passengers for flight
		logger.info(
			"Computed missed connecting passengers for flight %s: %d passengers.",
			flight_uid,
			len(pax_needing_reallocation),
			extra={
				"env_time": self.agent.env.now,
				'event_type': 'PassengerImpact',
				'details': {
					'flight_number': flight_uid,
					'missed_connections': len(pax_needing_reallocation),
					'action': 'Computed Missed Connecting Passengers'
				}
			}
		)
		##
	...
	def reallocate_pax(self, pax, reallocation_options):
		...
		#Integrating logging capabilities for the agent
		#capturing Reallocated passenger for flight
		logger.info(
            "Reallocated passenger %s for flight %s with options: %s.",
            pax.id,  
            self.agent.uid,
            ...
            extra={
                ...
                'details': {
                    'flight_number': self.agent.uid,
                    'passenger_id': pax.id,
                    'reallocation_options': reallocation_options,
                    'action': 'Reallocated Passenger'
                }
            }
        )
        ##
	...
	def send_allocation_pax_request(self, pax, aoc_uid, flight_uid):
		...
		#Integrating logging capabilities for the agent
		#capturing Sent allocation request for passenger
		logger.info(
            "Sent allocation request for passenger %s to AOC %s for flight %s.",
            pax.id,  
            aoc_uid,
            flight_uid,
            extra={
                ...
                'details': {
                    ...
                    'passenger_id': pax.id,
                    'aoc_uid': aoc_uid,
                    'action': 'Sent Allocation Pax Request'
                }
            }
        )
        ##
	...
	def send_remove_pax_from_boarding_list_request(self, pax, flight_uid):
		...
		#Integrating logging capabilities for the agent
		#capturing Sent remove pax request for passenger
		logger.info(
            "Sent remove pax request for passenger %s from boarding list for flight %s.",
            pax.id,  
            flight_uid,
            extra={
				"env_time": self.agent.env.now,
                ...
                'details': {
                    'flight_number': flight_uid,
                    'passenger_id': pax.id,
                    'action': 'Sent Remove Pax from Boarding List Request'
                }
            }
        )
        ##
	...
	def wait_for_remove_pax_from_boarding_list_request(self, msg):
		...
		#Integrating logging capabilities for the agent
		#capturing Received remove pax request for passenger
		pax = msg['body'].get('pax', None)
		flight_uid=msg['body'].get('flight_uid', None)
		logger.info(
            "Received confirmation to remove passenger %s from boarding list for flight %s.",
            pax.id,  
            flight_uid,
            extra={
				"env_time": self.agent.env.now,
                ...
                'details': {
                    'flight_number': flight_uid,
                    'passenger_id': pax.id,
                    'action': 'Received Remove Pax from Boarding List Confirmation'
                }
            }
        )
        ##
	...
	def wait_for_allocation_pax_request(self, msg):
		...
		#Integrating logging capabilities for the agent
		#capturing Received allocation pax request for passenger
		pax = msg['body'].get('pax', None)
		flight_uid=msg['body'].get('flight_uid', None)
		logger.info(
            "Received allocation pax request for passenger %s for flight %s from AOC %s.",
            pax.id, 
            flight_uid,
            msg['from'],
            extra={
				...
				'details': {
					...
					'passenger_id': pax.id,
					'aoc_uid': msg['from'],
					'action': 'Received Allocation Pax Request'
				}
            }
        )
        ##
	...
	def wait_for_reallocation_pax_request(self, msg):
		...
		#Integrating logging capabilities for the agent
		#capturing Received reallocation pax request for passenger
		paxs = msg['body'].get('paxs', [])
		pax_ids = ', '.join([str(pax.id) for pax in paxs])
		flight_uid=msg['body'].get('flight_uid', None)
		logger.info(
            "Received reallocation pax request for passenger %s for flight %s from AOC %s.",
            pax_ids, 
            flight_uid,
            msg['from'],
            extra={
                ...
                'details': {
					...
					'aoc_uid': msg['from'],
                    'action': 'Received Reallocation Pax Request'
                }
            }
        )
        ##
	...
	def wait_for_reallocation_options_request(self, msg):
		...
		#Integrating logging capabilities for the agent
		#capturing Received reallocation options request for passenger
		pax = msg['body'].get('pax', None)
		flight_uid=msg['body'].get('flight_uid', None)
		logger.info(
            "Received reallocation options request for passenger %s for flight %s from AOC %s.",
            pax.id, 
            flight_uid,
            msg['from'],
            extra={
                ...
                'details': {
					...
					'passenger_id': pax.id,
					'aoc_uid': msg['from'],
                    'action': 'Received Reallocation Options Request'
                }
            }
        )
        ##
	...
	def return_reallocation_options(self, pax, options, pax_handler_uid):
		...
		#Integrating logging capabilities for the agent
		#capturing Returned reallocation options for passenger
		logger.info(
            "Returned reallocation options for passenger %s for flight %s to Pax Handler UID %s.",
            pax.id,  # Assuming pax has an 'id' attribute
            self.agent.uid,
            pax_handler_uid,
            extra={
				"env_time": self.agent.env.now,
                ...
                'details': {
                    'flight_number': self.agent.uid,
                    'passenger_id': pax.id,
                    'pax_handler_uid': pax_handler_uid,
                    'reallocation_options': options,
                    'action': 'Returned Reallocation Options'
                }
            }
        )
        ##
...
class TurnaroundOperations(Role):
	...
	def process_following_flight(self, flight_uid):
		...
		#Integrating logging capabilities for the agent
		#capturing Processed following flight for flight
		logger.info(
            "Processing following flight for flight %s.",
            flight_uid,
            extra={
				"env_time": self.agent.env.now,
                ...
                'details': {
                    'flight_number': flight_uid,
                    'action': 'Processed Following Flight'
                }
            }
        )
        ##
	def request_process_arrival_pax(self, flight_uid):
		...
		#Integrating logging capabilities for the agent
		#capturing Requested processing of arriving passengers for flight
		logger.info(
            "Requested processing of arriving passengers for flight %s.",
            flight_uid,
            extra={
				"env_time": self.agent.env.now,
                'event_type': 'PassengerImpact',
                'details': {
                    'flight_number': flight_uid,
                    'action': 'Requested Processing of Arriving Passengers'
                }
            }
        )
        ##
	...
	def request_turnaround(self, aircraft, last_flight_uid, next_flight_uid):
		...
		#Integrating logging capabilities for the agent
		#capturing Requested turnaround for aircraft
		logger.info(
            "Requested turnaround for aircraft %s between flights %s and %s.",
            aircraft.idd,  
            last_flight_uid,
            next_flight_uid,
            extra={
                ...
                'event_type': 'Turnaround',
                'details': {
                    'aircraft_id': aircraft.idd,
                    'last_flight_uid': last_flight_uid,
                    'next_flight_uid': next_flight_uid,
                    'action': 'Requested Turnaround'
                }
            }
        )
        ##
	...
	def request_departing_reassessment(self, flight_uid, ac_ready_at_time):
		...
		#Integrating logging capabilities for the agent
		#capturing Requested departing reassessment for flight
		logger.info(
            "Requested departing reassessment for flight %s with aircraft ready at %s.",
            flight_uid,
            ac_ready_at_time,
            extra={
				"env_time": self.agent.env.now,
                ...
                'details': {
                    'flight_number': flight_uid,
                    'aircraft_ready_at': ac_ready_at_time,
                    'action': 'Requested Departing Reassessment'
                }
            }
        )
        ##
...
class AirlinePaxHandler(Role):
	...
	def _assume_cost(self, flight_uid, cost, cost_type):
		...
		#Integrating logging capabilities for the agent
		#capturing Assumed cost of type for flight
		logger.info(
			"Assumed cost of type '%s' amounting to %s for flight %s.",
			cost_type,
			cost,
			flight_uid,
			extra={
	...
	def check_delay_liability(self, pax, current_flight_uid=None):
		...
		#Integrating logging capabilities for the agent
		#capturing Checked delay liability for passenger
		logger.info(
			"Checked delay liability for passenger %s on flight %s.",
			pax.id,  # Assuming pax has an 'id' attribute
			current_flight_uid or self.agent.uid,
			extra={
				"env_time": self.agent.env.now,
				'event_type': 'DelayLiability',
				'details': {
					'flight_number': current_flight_uid or self.agent.uid,
					'passenger_id': pax.id,
					'action': 'Checked Delay Liability'
				}
			}
		)
		##
	...
	def do_pax_care(self, delay, pax):
		...
		#Integrating logging capabilities for the agent
		#capturing Provided care for passenger
		logger.info(
			"Provided care for passenger %s due to delay of %s minutes on flight %s.",
			pax.id,  # Assuming pax has an 'id' attribute
			delay,
			self.agent.uid,
			extra={
				...
				'details': {
					...
					'passenger_id': pax.id,
					'delay_minutes': delay,
					'action': 'Provided Passenger Care'
				}
			}
		)
		##
	...
	def board_pax(self, flight_uid):
		...
		#Integrating logging capabilities for the agent
		#capturing Boarded passengers for flight
		logger.info(
            "Boarded passengers for flight %s.",
            flight_uid,
            extra={
				"env_time": self.agent.env.now,
                'event_type': 'PassengerBoarding',
                'details': {
                    'flight_number': flight_uid,
                    'action': 'Boarded Passengers'
                }
            }
        )
        ##
	...
	def arrive_pax(self, pax, overnight=False):
		...
		#Integrating logging capabilities for the agent
		#capturing Arrived passenger on flight
		logger.info(
            "Passenger %s arrived on flight %s%s.",
            pax.id, 
            self.agent.uid,
            " overnight" if overnight else "",
            extra={
				"env_time": self.agent.env.now,
	...
	def find_blame_for_compensation_and_soft_cost(self, pax, compensation, soft_cost):
		...
		#Integrating logging capabilities for the agent
		#capturing Compensation for passenger
		logger.info(
            "Found blame for passenger %s: Compensation=%s, Soft Cost=%s for flight %s.",
            pax.id,  
            compensation,
            soft_cost,
            self.agent.uid,
            extra={
                ...
                'details': {
                    ...
                    'soft_cost': soft_cost,
                    'action': 'Found Blame for Compensation and Soft Cost'
                }
            }
        )
        ##
	def pay_pax_cost(self, pax, cost, cost_type, flight_uid=None):
		if cost > 0.:
			...
			#Integrating logging capabilities for the agent
			#capturing Paid passenger cost for flight
			logger.info(
				"Paid %s of %s to passenger %s for flight %s.",
				cost_type,
				cost,
				pax.id,  
				flight_uid,
	...
	def follow_blame_flight(self, flight_uid, pax, compensation, soft_cost):
		...
		#capturing Compensation for passenger
		logger.info(
            "Followed blame for passenger %s on flight %s: Compensation=%s, Soft Cost=%s.",
            pax.id, 
            flight_uid,
            compensation,
            soft_cost,
            extra={
                ...
                'details': {
                    ...
                    'compensation': compensation,
                    'soft_cost': soft_cost,
                    'action': 'Followed Blame for Flight'
                }
            }
        )
        ##
	...
	def request_connecting_times(self, pax, connection_type):
		...
		#Integrating logging capabilities for the agent
		#capturing Requested connecting times for passenger
		logger.info(
            "Requested connecting times for passenger %s with connection type '%s'.",
            pax.id, 
            connection_type,
            extra={
				"env_time": self.agent.env.now,
                ...
                'details': {
                    'passenger_id': pax.id,
                    'connection_type': connection_type,
                    'action': 'Requested Connecting Times'
                }
            }
        )
        ##
	...
	def request_pax_connection_handling(self, paxs):
        ...
        #Integrating logging capabilities for the agent
		#capturing Requested connection handling for passengers
		logger.info(
            "Requested connection handling for passengers: %s.",
            [pax.id for pax in paxs], 
            extra={
				"env_time": self.agent.env.now,
                'event_type': 'PassengerImpact',
                'details': {
                    'passengers_ids': [pax.id for pax in paxs],
                    'action': 'Requested Passenger Connection Handling'
                }
            }
        )
        ##
	...
	def request_pax_rail_connection_handling(self, pax):
		...
		#Integrating logging capabilities for the agent
		#capturing Requested rail connection handling for passenger
		logger.info(
            "Requested rail connection handling for passenger %s.",
            pax.id,  
            extra={
				"env_time": self.agent.env.now,
                'event_type': 'PassengerImpact',
                'details': {
                    'passenger_id': pax.id,
                    'action': 'Requested Rail Connection Handling'
                }
            }
        )
        ##
	...
	def handle_pax_connection(self, paxs):
		...
		#Integrating logging capabilities for the agent
		#capturing Handled connections for passengers
		logger.info(
            "Handled connections for passengers: %s.",
            [pax.id for pax in paxs],  
            extra={
				"env_time": self.agent.env.now,
                'event_type': 'PassengerImpact',
                'details': {
                    'passengers_ids': [pax.id for pax in paxs],
                    'action': 'Handled Passenger Connections'
                }
            }
        )
        ##
	...
	def send_cost_blame(self, flight_uid, cost, cost_type):
		...
		#Integrating logging capabilities for the agent
		#capturing Sent cost blame for flight
		logger.info(
            "Sent cost blame for %s of %s to flight %s.",
            cost_type,
            cost,
            flight_uid,
            extra={
	...
	def send_follow_blame_request(self, flight_uid, pax, compensation, soft_cost):
		...
		#Integrating logging capabilities for the agent
		#capturing Sent follow blame request
		logger.info(
            "Sent follow blame request for passenger %s to flight %s with Compensation=%s and Soft Cost=%s.",
            pax.id,  
            flight_uid,
            compensation,
            soft_cost,
            extra={
                ...
                'details': {
                    ...
                    'compensation': compensation,
                    'soft_cost': soft_cost,
                    'action': 'Sent Follow Blame Request'
                }
            }
        )
        ##
	...
	def wait_for_connecting_times(self, msg):
		...
		#Integrating logging capabilities for the agent
		#capturing Received connecting times for passenger	
		flight_uid=msg['body'].get('flight_uid', None)
		logger.info(
			"Received connecting times for passenger %s on flight %s.",
			pax.id, 
			flight_uid,
			extra={
				"env_time": self.agent.env.now,
				...
				'details': {
					'flight_number': flight_uid,
					'passenger_id': pax.id,
					'action': 'Received Connecting Times'
				}
			}
		)
        ##
	...
	def wait_for_cost_blame(self, msg):
		...
		#Integrating logging capabilities for the agent
		#capturing Received cost blame for flight
		logger.info(
            "Received cost blame for flight %s: Cost Type=%s, Amount=%s.",
            msg['body']['flight_uid'],
            msg['body']['cost_type'],
            msg['body']['cost'],
            extra={
                ...
                'event_type': 'Cost',
                'details': {
                    'flight_number': msg['body']['flight_uid'],
                    'cost_type': msg['body']['cost_type'],
                    'cost_amount': msg['body']['cost'],
                    'action': 'Received Cost Blame'
                }
            }
        )
        ##
	...
	def wait_for_follow_blame(self, msg):
		...
		#Integrating logging capabilities for the agent
		#capturing Received follow blame for passenger
		pax = msg['body'].get('pax', None)
		flight_uid=msg['body'].get('flight_uid', None)
		compensation=msg['body'].get('compensation', None)
		soft_cost=msg['body'].get('soft_cost', None)
		logger.info(
            "Received follow blame for passenger %s on flight %s: Compensation=%s, Soft Cost=%s.",
            pax.id,  
            flight_uid,
            compensation,
            soft_cost,
	...
	def wait_for_pax_connection_handling_request(self, msg):
		...
		#Integrating logging capabilities for the agent
		#capturing Received passenger connection handling request
		paxs = msg['body']['paxs']
		pax_ids = [pax.id for pax in paxs]  
		num_paxs = len(paxs)
		flight_uid=msg['body'].get('flight_uid', None)
		logger.info(
			"Received passenger connection handling request for %s passengers on flight %s.",
			num_paxs, 
			flight_uid,
			extra={
				"env_time": self.agent.env.now,
				...
				'details': {
					...
					'number_of_passengers': num_paxs,
					'action': 'Received Passenger Connection Handling Request'
				}
			}
		)
        ##
	def wait_for_process_arrival_pax_request(self, msg):
		...
		#Integrating logging capabilities for the agent
		#capturing Processed arrival pax request for flight
		logger.info(
            "Received process arrival pax request for flight %s.",
            flight_uid,
            extra={
				"env_time": self.agent.env.now,
                'event_type': 'PassengerImpact',
                'details': {
                    'flight_number': flight_uid,
                    'action': 'Received Process Arrival Pax Request'
                }
            }
        )
        ##
	def wait_for_time_at_gate_update_in_aoc_request(self, msg):
		...
		#Integrating logging capabilities for the agent
		#capturing Updated time at gate for passenger
		logger.info(
            "Updated time at gate for passenger %s on flight %s. New time at gate: %s minutes.",
            pax_id, 
            flight_uid, 
            time_at_gate,
            extra={
