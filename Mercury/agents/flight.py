...
class Flight(Agent):
	...
	def __init__(self, *args, **kwargs):
		...
		# 		logger.warning(
		# 			f"Attribute '{attr}' not provided. Using default value: {default}",
		# 			extra={
...
class AircraftDepartingHandler(Role):
	...
	def do_taxi_out(self):
		...
		##Helps logging 
		TaxiOutStart = self.agent.env.now
		##
		...
		#Integrating logging capabilities for the agent capabilities
		#capturing the actual taxi-out time and comparing it with the planned taxi-out time
		# Taxi-out done
		TaxiOutActualDuration = self.agent.env.now - TaxiOutStart
		PlannedTaxiOutDuration = self.agent.FP.atot - self.agent.FP.aobt if self.agent.FP.aobt else None

		# LOGGING STATEMENT: Compare actual taxi-out time vs. planned
		if PlannedTaxiOutDuration is not None:
			logger.info(
				"Flight %s TAXI-OUT COMPLETED. ActualDuration=%.2f, Planned=%.2f, ExtraDelay=%.2f",
				self.agent.uid,
				TaxiOutActualDuration,
				PlannedTaxiOutDuration,
				TaxiOutActualDuration - PlannedTaxiOutDuration,
				extra={
					"env_time": self.agent.env.now,
					'event_type': 'GroundOperation',
					'details': {
						'flight_number': self.agent.uid,
						'actual_duration': TaxiOutActualDuration,
						'planned_duration': PlannedTaxiOutDuration,
						'extra_delay': TaxiOutActualDuration - PlannedTaxiOutDuration,
						'action': 'Taxi-Out Completed'
					}
				}
			)
		else:
			logger.info(
				"Flight %s TAXI-OUT COMPLETED with PlannedTaxiOutDuration not defined.",
				self.agent.uid,
				extra={
					"env_time": self.agent.env.now,
					'event_type': 'GroundOperation',
					'details': {
						'flight_number': self.agent.uid,
						'action': 'Taxi-Out Completed with undefined planned duration'
					}
				}
			)
	...
	def request_taxi_out_time(self):
		...
		#Integrating logging capabilities for the agent
		#capturing the actual taxi-out time for flight to airport
		logger.info(
            "Sent taxi-out time request for flight %s to airport %s.",
            self.agent.uid,
            self.agent.origin_airport_uid,
            extra={
				"env_time": self.agent.env.now,
                'event_type': 'GroundOperation',
                'details': {
                    'flight_number': self.agent.uid,
                    'airport_uid': self.agent.origin_airport_uid,
                    'action': 'Sent Taxi-Out Time Request'
                }
            }
        )
		##
	...
	def wait_for_taxi_out_time(self, msg):
		...
		#Integrating logging capabilities for the agent
		#capturing Received taxi-out time estimation for flight
		logger.info(
            "Received taxi-out time estimation for flight %s: %s minutes.",
            self.agent.uid,
            self.agent.FP.axot,
            extra={
				"env_time": self.agent.env.now,
                ...
                'details': {
                    'flight_number': self.agent.uid,
                    'taxi_out_time_estimation': self.agent.FP.axot,
                    'action': 'Received Taxi-Out Time Estimation'
                }
            }
        )
		##
...
class DepartureSlotRequester(Role):
	...
	def request_departure_slot(self):
		...
		#Integrating logging capabilities for the agent
		#capturing the departure slot request for the flight
		logger.info(
            "Sent departure slot request for flight %s with estimated takeoff time %s.",
            self.agent.uid,
            self.agent.FP.get_estimated_takeoff_time(),
            extra={
				"env_time": self.agent.env.now,
                'event_type': 'Departure',
                'details': {
                    'flight_number': self.agent.uid,
                    'estimated_takeoff_time': self.agent.FP.get_estimated_takeoff_time(),
                    'action': 'Sent Departure Slot Request'
                }
            }
        )
		##
	...
	def request_taxi_out_time_estimation(self):
		...
		#Integrating logging capabilities for the agent
		#capturing the taxi-out time estimation request for the flight
		logger.info(
            "Sent taxi-out time estimation request for flight %s to airport %s.",
            self.agent.uid,
            self.agent.origin_airport_uid,
            extra={
				"env_time": self.agent.env.now,
                ...
                'details': {
                    'flight_number': self.agent.uid,
                    'airport_uid': self.agent.origin_airport_uid,
                    'action': 'Sent Taxi-Out Time Estimation Request'
                }
            }
        )
		##
	...
	def wait_for_taxi_out_time_estimation(self, msg):
		...
		#Integrating logging capabilities for the agent
		#capturing the taxi-out time estimation for the flight
		logger.info(
            "Received taxi-out time estimation for flight %s: %s minutes.",
            self.agent.uid,
            self.agent.FP.axot,
	...
	def wait_for_departure_slot(self, msg):
		...
		#Integrating logging capabilities for the agent
		#capturing the departure slot for the flight
		logger.info(
            "Received departure slot for flight %s: %s.",
            self.agent.uid,
            self.departing_slot,
            extra={
				"env_time": self.agent.env.now,
                ...
                'details': {
                    'flight_number': self.agent.uid,
                    'departure_slot': self.departing_slot,
                    'action': 'Received Departure Slot'
                }
            }
        )
		##
	...
	def compute_push_back_time(self, slot):
		...
		#Integrating logging capabilities for the agent
		#capturing the computed push back time for the flight
		logger.info(
            "Computed push back time for flight %s: EIBT=%s, AOBT=%s.",
            self.agent.uid,
            self.agent.FP.eibt,
            self.agent.FP.aobt,
            extra={
                ...
                'details': {
                    'flight_number': self.agent.uid,
                    'EIBT': self.agent.FP.eibt,
                    'AOBT': self.agent.FP.aobt,
                    'action': 'Computed Push Back Time'
                }
            }
        )
		##
...
class FlightPlanUpdater(Role):
	...
	def update_speed(self, perc_selected, change_tod_dci=True):
		...
		#Integrating logging capabilities for the agent
		#capturing the updated speed for the flight
		logger.info(
			"Updated speed for flight %s to %s%%.",
			self.agent.uid,
			perc_selected,
			extra={
				"env_time": self.agent.env.now,
	...
	def update_eobt(self, eobt, push_back_ready_event):
		...
		#Integrating logging capabilities for the agent
		#capturing the updated EOBT for the flight
		logger.info(
			"Updated EOBT for flight %s to %s.",
			self.agent.uid,
			eobt,
			extra={
				"env_time": self.agent.env.now,
...
class FlightArrivalInformationProvider(Role):
	...
	def provide_flight_arrival_information(self, fai, eaman_uid):
		...
		#Integrating logging capabilities for the agent
		#capturing the provided flight arrival information for the flight
		logger.info(
			"Provided flight arrival information for flight %s to EAMAN %s.",
			self.agent.uid,
			eaman_uid,
			extra={
				"env_time": self.agent.env.now,
				'event_type': 'FlightArrival',
				'details': {
					'flight_number': self.agent.uid,
					'eaman_uid': eaman_uid,
					'fai': fai,
					'action': 'Provided Flight Arrival Information'
				}
			}
		)
		##
...
class GroundArrivalHandler(Role):
	...
	def do_taxi_in(self):
		...
		#Integrating logging capabilities for the agent
		#capturing the taxi-in time for the flight
		FinalArrivalTime = self.agent.env.now
		plannedSibt = self.agent.sibt
		if plannedSibt is not None:
			delay = FinalArrivalTime - plannedSibt
			if delay > 0:
				logger.info(
					"Flight %s arrived with a delay of %.2f minutes.",
					self.agent.uid,
					delay,
					extra={
						"env_time": self.agent.env.now,
						...
						'details': {
							'flight_number': self.agent.uid,
							'final_arrival_time': FinalArrivalTime,
							'planned_sibt': plannedSibt,
							'delay': delay,
							'action': 'Arrival Delay'
						}
					}
				)
			else:
				logger.info(
					"Flight %s arrived on time.",
					self.agent.uid,
					extra={
						"env_time": self.agent.env.now,
						'event_type': 'Arrival',
						'details': {
							...
							'final_arrival_time': FinalArrivalTime,
							'planned_sibt': plannedSibt,
							'action': 'On-Time Arrival'
						}
					}
				)
		else:
			logger.info(
				"Flight %s arrived without a planned SIBT.",
				self.agent.uid,
				extra={
					"env_time": self.agent.env.now,
					'event_type': 'Arrival',
					'details': {
						'flight_number': self.agent.uid,
						'final_arrival_time': FinalArrivalTime,
						'action': 'Arrival without Planned SIBT'
					}
				}
			)
		##
...
class PotentialDelayRecoveryProvider(Role):
	...
	def compute_potential_delay_recovery(self, force_compute=False, use_dci_landing=True):
		...
		#Integrating logging capabilities for the agent
		#capturing the computed potential delay recovery for the flight
		logger.info(
			"Computed potential delay recovery for flight %s: %s.",
			self.agent.uid,
			tfsc,
			extra={
				"env_time": self.agent.env.now,
				...
				'details': {
					'flight_number': self.agent.uid,
					'computed_recovery': tfsc,
					'force_compute': force_compute,
					'use_dci_landing': use_dci_landing,
					'action': 'Computed Potential Delay Recovery'
				}
			}
		)
		##
	...
	def provide_potential_delay_recover_information(self, potential_delay_recovery, to_uid):
		...
		#Integrating logging capabilities for the agent
		#capturing the provided potential delay recovery information for the flight
		logger.info(
			"Provided potential delay recovery information for flight %s to UID %s.",
			self.agent.uid,
			to_uid,
			extra={
				"env_time": self.agent.env.now,
				...
				'details': {
					'flight_number': self.agent.uid,
					'to_uid': to_uid,
					'potential_delay_recovery': potential_delay_recovery,
					'action': 'Provided Potential Delay Recovery Information'
				}
			}
		)
		##
